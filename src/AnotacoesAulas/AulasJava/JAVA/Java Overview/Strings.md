...
public class MetodoCharAt {
    public static void main(String[] args) {
        ...
        for (int i = 0; i < java.length(); i++) {  // length()
...
Como visto em `[[Java-String-Construtores-e-Memoria]]`, o compilador Java mantém um **String Pool**. Ao detectar que `"Ola"` já existe no pool, ele **não cria um segundo objeto** — `ola3` aponta para o mesmo endereço de `ola`.
...
> ⚠️ **Correção no código original:** `a.compareTo('a')` está errado — `compareTo()` recebe uma `String`, não um `char`. O correto seria `a.compareTo("a")`.
...
# String Caixa alta o u Minuscula

``` java
public class LowerCaseUpperCase{
	public static void main(String[] arg){
		String teste = "Isso E UM TEste";
		String testeMinuscula = teste.toLowerCase();
		String testeMaiuscula = teste.toUpperCase();
